                       ...
                       const int& finish_x, const int& finish_y)
{
    ...
    Board_Type board(dim_x + 2, std::vector<int>(dim_y + 2, 0));
                     ...
                     const int& current_x, const int& current_y)
{
    ...
    // this keeps track of the number of possible hsw's
    int total = 0;

    // check adjacent spaces for available moves
    for(int i = -1; i < 2; ++i)
    {
        for(int j = -1; j < 2; ++j)
        {
            ...
            {
                // add up successful hsw's
                total += countHSW_recurse(board,
                                          current_x + i, current_y + j);
            }
        }
    }
    // reset current position to 0 so backtracking works
